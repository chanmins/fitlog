...
(() => {
  ...
  /* ── Render Root ──────────────────────────── */
  /* 시트는 render() 가 appEl 을 통째로 다시 그릴 때마다 DOM 노드째 새로
     태어납니다. 그래서 시트가 떠 있는 동안 상태가 바뀌면(부위를 하나 더
     고른다든가) 새로 만들어진 .sheet-panel 에서 sheetUp 이 처음부터 다시
     재생돼, 시트가 화면 밖까지 내려갔다 도로 올라옵니다.

     운동 선택 시트는 이 문제를 피하려고 render() 대신 그 줄만 다시 칠하는
     길(paintPickRow)을 따로 냈습니다. 하지만 부위 시트는 그 방법을 쓸 수
     없습니다 — 부위를 끄면 뒤에 깔린 기록 화면의 운동 카드까지 사라져야
     하므로 어차피 전체를 다시 그려야 합니다. 그래서 다시 그리되, '이번
     렌더 이전에도 이미 떠 있던 시트' 에서는 등장 애니메이션만 끕니다.
     .detail-screen 의 스크롤을 _overlayKey 로 다루는 것과 같은 방식입니다.

     같은 이유로 시트 안의 스크롤 위치도 여기서 옮겨 줍니다. 새 노드는
     scrollTop 이 0 이라, 긴 목록을 내려 보다 무언가를 누르면 목록이
     맨 위로 튀어 버립니다. */
  let _prevSheets = new Set();
  let _openSheets = new Set();
  const _sheetScroll = new Map();
  function sheetAttrs(id) {
    _openSheets.add(id);
    return `class="sheet-backdrop${_prevSheets.has(id) ? ' no-enter' : ''}" data-sheet="${esc(id)}"`;
  }
  function saveSheetScroll() {
    _sheetScroll.clear();
    document.querySelectorAll('.sheet-backdrop').forEach(bd => {
      const panel = bd.querySelector('.sheet-panel');
      if (panel && panel.scrollTop) _sheetScroll.set(bd.dataset.sheet || '', panel.scrollTop);
    });
  }
  function restoreSheetScroll() {
    if (!_sheetScroll.size) return;
    document.querySelectorAll('.sheet-backdrop').forEach(bd => {
      const top = _sheetScroll.get(bd.dataset.sheet || '');
      const panel = bd.querySelector('.sheet-panel');
      if (top && panel) panel.scrollTop = top;
    });
    _sheetScroll.clear();
  }
  function render() {
  ...
  function renderExerciseInfoSheet(exId) {
    ...
    return `<div ${sheetAttrs(`info:${exId}`)}>
      <div class="sheet-panel">
        <div class="sheet-grab"></div>
        <div class="sheet-head">
          <div class="info-hero">
  ...
  function renderExercisePickerSheet(partId) {
    ...
    const n = state.pickSelection.length;
    return `<div ${sheetAttrs(`pick:${partId}`)}>
  ...
  function renderRoutineSheet() {
    ...
    return `<div ${sheetAttrs('routines')}>
      <div class="sheet-panel">
        <div class="sheet-grab"></div>
        <div class="sheet-head">
          <div>
